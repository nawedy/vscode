 ...
 * Kimi Provider
 *
 * Provider implementation for MoonShot AI's Kimi API integration.
 * Supports Kimi models for code generation and completion.
 */
...
interface KimiCompletionRequest {
	model: string;
...
/**
 * Provider for Kimi models
// @ts-ignore: error TS2415: Class 'KimiProvider' incorrectly extends base class 'BaseModelProvider'.
// @ts-ignore: error TS2415: Class 'KimiProvider' incorrectly extends base class 'BaseModelProvider'.
// @ts-ignore: error TS2415: Class 'KimiProvider' incorrectly extends base class 'BaseModelProvider'.
// @ts-ignore: error TS2415: Class 'KimiProvider' incorrectly extends base class 'BaseModelProvider'.
// @ts-ignore: error TS2415: Class 'KimiProvider' incorrectly extends base class 'BaseModelProvider'.
...
export class KimiProvider extends BaseModelProvider {
	...
	async initialize(): Promise<boolean> {
		try {
			this.logger.info('Initializing Kimi provider');

			// Get API key
			this.apiKey = await this.configService.getSecret('kimi.apiKey') || '';
			if (!this.apiKey) {
				this.logger.warn('Kimi API key not found');
				return false;
			}
			...
			this.client = axios.create({
				...
				timeout: this.configService.get<number>('providers.kimi.timeout', 30000)
			});
			...
			this._isReady = true;
			this.logger.info('Kimi provider initialized successfully');
			return true;
		} catch (error) {
			this.logger.error(`Failed to initialize Kimi provider: ${error instanceof Error ? error.message : String(error)}`);
			return false;
		}
	}
			...
			if (this.client) {
				try {
					...
					for (const model of apiModels) {
						...
						// Only add models that are related to Kimi
						if (model.id.includes('moonshot')) {
					...
					this.logger.warn(`Failed to get models from Kimi API: ${error instanceof Error ? error.message : String(error)}`);
				}
			}

			this.logger.info(`Loaded ${this.models.size} Kimi models`);
		} catch (error) {
			throw new Error(`Failed to load Kimi models: ${error instanceof Error ? error.message : String(error)}`);
		}
	}
	...
	async generateCompletion(prompt: string, options?: ModelRequestOptions): Promise<ModelResponse> {
		if (!this.isReady || !this.client) {
			throw new ModelProviderError('Kimi provider not initialized', this.id);
		}
		...
		try {
			...
			// Prepare request
			const request: KimiCompletionRequest = {
				model: modelId,
				messages,
			...
			const response = await this.client.post<KimiCompletionResponse>('/chat/completions', request);
...
// @ts-ignore: error TS2339: Property 'error' does not exist on type 'unknown'.
				throw new ModelProviderError(
					`Kimi request failed: ${error.response?.data?.error?.message || error.message}`,
					this.id,
					modelId,
					String(error.response?.status || 'NETWORK_ERROR')
				);
			}
			throw new ModelProviderError(`Kimi request failed: ${error instanceof Error ? error.message : String(error)}`, this.id, modelId);
		}
	}
			...
			response.data.on('data', (chunk: Buffer) => {
				try {
					...
					for (const line of lines) {
						...
						const data = JSON.parse(dataMatch[1]) as KimiStreamChunk;
...
// @ts-ignore: error TS2339: Property 'error' does not exist on type 'unknown'.
					new ModelProviderError(
						`Kimi streaming failed: ${error.response?.data?.error?.message || error.message}`,
						this.id,
						modelId,
						String(error.response?.status || 'NETWORK_ERROR')
					)
				...
				handler.onError(
					new ModelProviderError(
						`Kimi streaming failed: ${error instanceof Error ? error.message : String(error)}`,
						this.id,
						modelId
					)
				);
	...
	private inferModelCapabilities(modelId: string): ModelCapability[] {
		...
		// All Kimi models support basic chat completion
		capabilities.push(ModelCapability.ChatCompletion);

		if (lowerModelId.includes('128k')) {
			// Larger context models have more capabilities
			...
			// Basic capabilities for smaller models
			capabilities.push(ModelCapability.CodeCompletion);
			capabilities.push(ModelCapability.Explanation);
		}
	...
	private getContextLengthForModel(modelId: string): number {
		...
		if (lowerModelId.includes('128k')) {
			...
			return 8192; // Default for standard models
		}
	}
}
